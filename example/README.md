...
1. **准备配置文件**:
   复制 `.env.example` 为 `.env` 并填入你需要启用的 OAuth 提供商的凭据 (Client ID / Client Secret)。
...
3. **测试端点**:
   - 首页: 访问 [http://127.0.0.1:8080/](http://127.0.0.1:8080/)，包含所有可用提供商的登录链接。
   - 绑定页面（针对已登录用户，此操作需要附带有效 `jwt_token` Cookie 或者是 Authorization Bearer头）:
     ```
     GET http://127.0.0.1:8080/api/v1/oauth/{provider}/bind
     ```
